...
const STATUS_BADGE_CLASSES = {
  success: "bg-[#0e2a1d] border-[#1e5d3b] text-green-300",
  warn: "bg-[#3a2a0e] border-[#6a531e] text-yellow-300",
  error: "bg-[#2a1313] border-[#5d1e1e] text-red-300",
  default: "bg-[#182432] border-[#233046] text-gray-300",
};
...
export default function ProblemPage() {
  ...
  const showCompileErrors = useCallback(
    (errors) => {
      const editor = editorRef.current;
      const monaco = monacoRef.current;
      if (!editor || !monaco) return;
      ...
      const markers = errors.map((e) => ({
        ...
        severity:
          e.severity === "warning" ? monaco.MarkerSeverity.Warning : monaco.MarkerSeverity.Error,
      }));
  ...
  const processExecutionResult = useCallback(
    (payload) => {
      if (!payload) {
        ...
        appendConsole("No result was returned from the server.");
        return;
      }
    ...
    [appendConsole, showCompileErrors]
  );
  ...
  }, [runJobId, runJobQuery.data, problemId, posthog, queryClient, setRunResult, processExecutionResult, appendConsole]);
  ...
  useEffect(() => {
    ...
    if (state === "failed") {
      ...
      setStatusBadge({ type: "error", text: "Execution Failed" });
      appendConsole("Submission job failed.\n");
      setSubmitMeta(null);
      setIsSubmitting(false);
  ...
  }, [submitMeta, submitJobQuery.data, problemId, posthog, queryClient, setSubmitResult, processExecutionResult, appendConsole]);
  ...
  useEffect(() => {
    ...
    setSubmitMeta(null);
    setIsSubmitting(false);
  }, [submitMeta, submitStoredQuery.isError, submitStoredQuery.error, appendConsole]);

  useEffect(() => {
    if (!runJobId || !runJobQuery.isError) return;
    setStatusBadge({ type: "error", text: "Execution Failed" });
    appendConsole("Failed to get job status.\n");
    setRunJobId(null);
    setIsRunning(false);
    posthog.capture("run_failed", { problem_id: problemId, error: "poll_error" });
  }, [runJobId, runJobQuery.isError, problemId, posthog, appendConsole]);

  useEffect(() => {
    if (!submitMeta || !submitJobQuery.isError) return;
    setStatusBadge({ type: "error", text: "Execution Failed" });
    appendConsole("Failed to get submission job status.\n");
    setSubmitMeta(null);
    setIsSubmitting(false);
    posthog.capture("submission_failed", { problem_id: problemId, error: "poll_error" });
  }, [submitMeta, submitJobQuery.isError, problemId, posthog, appendConsole]);
  ...
  }, [problemId, hints.length, hintsLoading, posthog]);

  useEffect(() => {
    if (activeTab === "Hints") fetchHints();
  }, [activeTab, fetchHints]);
  ...
  const statusClass =
    STATUS_BADGE_CLASSES[statusBadge?.type] ?? STATUS_BADGE_CLASSES.default;
      ...
      <div className="hidden xl:flex w-2/5 min-w-[480px] max-w-[720px] flex-col border-r border-[#1b2330] shadow-lg bg-[#10151c]/80">
        ...
        <div className="mt-3 px-2">
          ...
          <div className="border border-[#233046] rounded-b rounded-tr bg-[#0f141b] p-5 h-[calc(100vh-170px)] overflow-y-auto">
            ...
            {activeTab === "Solutions" && <SolutionTab problem={problem} />}
            {activeTab === "Hints" && (
              <HintTab
                hints={hints}
            ...
            {activeTab === "Discussions" && <ProblemDiscussions problemId={problemId} />}
          </div>
        </div>
      </div>
        ...
        {auth.user ? (
          ...
          <div className="pointer-events-none absolute right-6 bottom-4 text-sm bg-[#f3f6fa]/80 text-gray-800">
            <Link to="/login">
              <button
                type="button"
                className="bg-indigo-600 px-4 py-2 text-sm font-semibold text-white shadow hover:bg-indigo-500"
              >
