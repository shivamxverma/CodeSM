...
export default function DiscussionPage() {
  ...
  const fetchDiscussions = async () => {
    setLoading(true);
    try {
      const res = await getAllDiscussion();
  ...
  const filteredDiscussions = useMemo(() => {
    ...
    return discussions.filter((d) => {
      ...
      const u = (d.user?.username || d.user?.name || "").toLowerCase();
      ...
      <div className="flex flex-col h-full animate-fade-in">
        <div className="flex items-center justify-between gap-3 mb-4">
          <button
            ...
            className="text-left text-sm text-blue-400 hover:text-blue-300 inline-flex items-center gap-1 w-fit"
          >
          ...
          <button
            ...
            className="text-xs text-gray-400 hover:text-gray-200 border border-[#233046] bg-[#0c1219] px-3 py-1.5 rounded-lg"
          >
            Cancel
          </button>
        </div>
        ...
        <form onSubmit={handleCreateDiscussion} className="space-y-4 flex flex-col flex-1">
          <div className="bg-[#0c1219] border border-[#233046] rounded-xl p-4">
            <label className="block text-sm font-medium text-gray-200 mb-1">Title</label>
            <input
              required
              value={title}
              onChange={(e) => setTitle(e.target.value)}
              className="w-full p-2.5 bg-[#10151c] border border-[#233046] focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500 rounded-lg text-sm text-gray-200"
              placeholder="What’s your question or topic?"
            />
            <div className="text-xs text-gray-500 mt-2">
              Keep it specific. Great titles get faster replies.
            </div>
          </div>

          <div className="flex-1 flex flex-col min-h-0 bg-[#0c1219] border border-[#233046] rounded-xl p-4">
            <label className="block text-sm font-medium text-gray-200 mb-1">Content</label>
            <textarea
              required
              value={content}
              onChange={(e) => setContent(e.target.value)}
              className="w-full flex-1 p-2.5 bg-[#10151c] border border-[#233046] focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500 rounded-lg text-sm text-gray-200 resize-none"
              placeholder="Add details, code, context, expected output, etc."
            />
          </div>

          <button
            type="submit"
            className="w-full py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-sm font-semibold transition-colors mt-2"
          >
            Post Discussion
          </button>
        </form>
    ...
    return (
      <div className="flex flex-col h-full animate-fade-in">
        <button
          ...
          className="text-left text-sm text-blue-400 hover:text-blue-300 mb-3 inline-flex items-center gap-1 w-fit"
        >
          &larr; Back to list
        </button>

        <div className="flex-1 overflow-y-auto space-y-4 pb-4 custom-scrollbar pr-1">
          <div className="bg-[#0c1219] p-4 rounded-xl border border-[#233046] shadow-sm">
            <h2 className="text-lg font-bold text-gray-200 leading-snug">
            ...
            <div className="text-xs text-gray-400 mt-1 mb-3 flex items-center gap-2">
              ...
              {activeDiscussion.problem?.title ? (
                <>
                  <span>•</span>
            ...
            <div className="flex items-center gap-6 mt-4 pt-3 border-t border-[#1b2330]">
              <button
                onClick={(e) => handleLike(activeDiscussion._id, e)}
              ...
              <button
                onClick={(e) => handleDislike(activeDiscussion._id, e)}
          ...
          <div className="space-y-3">
            <h3 className="text-sm font-semibold text-blue-300">
              Comments ({activeDiscussion.comments?.length || 0})
            </h3>
            ...
            {activeDiscussion.comments?.length === 0 ? (
              ...
              activeDiscussion.comments?.map((c, i) => (
                <div key={i} className="bg-[#10151c] p-3 rounded-xl border border-[#233046]">
                  <div className="text-xs text-blue-200 mb-1.5 font-medium">Anonymous</div>
                  <div className="text-sm text-gray-300 leading-relaxed">
                    {c.content?.content || c.content || c}
                  </div>
                </div>
              ))
            )}
        ...
        {user ? (
          <form onSubmit={handleCreateComment} className="mt-3 flex gap-2 shrink-0">
            <input
              ...
              className="flex-1 p-2 bg-[#0c1219] border border-[#233046] focus:border-blue-500 focus:outline-none rounded-xl text-sm text-gray-200"
          ...
          <div className="mt-3 text-sm text-gray-400 text-center shrink-0 border border-[#233046] bg-[#0c1219] p-2 rounded-xl">
            Log in to comment
          </div>
        )}
      </div>
      ...
      <div className="flex items-center justify-between gap-3 mb-3 shrink-0">
        <div className="flex items-center gap-3 min-w-0 flex-1">
          <div className="min-w-0">
            <div className="text-lg font-bold text-gray-100 leading-tight">Discussions</div>
            <div className="text-xs text-gray-400">All discussions from the community</div>
          </div>
        </div>
        ...
        {user ? (
          <button
            ...
            className="px-4 py-2 bg-[#1a2432] hover:bg-[#1f2c3e] border border-[#2a3750] text-blue-300 rounded-lg text-sm font-semibold transition-colors inline-flex items-center justify-center gap-2"
          >
            <svg
              className="w-4 h-4"
              fill="none"
          ...
          <div className="px-4 py-2 bg-[#0c1219] border border-[#233046] text-gray-400 rounded-lg text-sm font-medium">
            Log in to create
          </div>
        )}
      </div>
      ...
      <div className="flex items-center gap-2 mb-3 shrink-0">
        <div className="flex-1 relative">
          <input
            ...
            className="w-full p-2.5 pl-9 bg-[#0c1219] border border-[#233046] focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500 rounded-xl text-sm text-gray-200"
        ...
        <button
          ...
          className="px-3 py-2.5 bg-[#0c1219] border border-[#233046] hover:border-[#3a4f73] text-gray-300 rounded-xl text-sm transition-colors"
          title="Refresh"
        >
          Refresh
        </button>
      ...
      <div className="flex-1 overflow-y-auto space-y-3 pb-2 pr-1 custom-scrollbar">
        {filteredDiscussions.length === 0 ? (
          ...
          filteredDiscussions.map((d) => (
            <div
              key={d._id}
              className="bg-[#0c1219] p-4 rounded-xl border border-[#233046] cursor-pointer hover:border-[#3a4f73] transition-colors shadow-sm"
              onClick={() => {
                setActiveDiscussion(d);
                setView("view");
              }}
            >
              <h3 className="font-semibold text-gray-200 text-base leading-tight mb-1">
                {d.title}
              </h3>
              ...
              <div className="flex items-center gap-3 mt-3 text-xs text-gray-500 border-t border-[#1b2330] pt-2.5">
                <span className="font-medium text-blue-200/70 truncate max-w-[140px]">
