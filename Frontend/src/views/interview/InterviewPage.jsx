...
const InterviewAssistant = () => {
    ...
    const handleStopRecording = () => {
        ...
        if (audioStreamRef.current) {
            ...
            audioStreamRef.current = null; // Clear the reference after stopping
        }
        setIsRecording(false);
    };
    ...
    const startTimer = () => {
        timerRef.current = window.setInterval(() => {
            setTimer((prev) => {
                if (
                    prev + 1 >=
                    JSON.parse(90) // Maximum recording time of 30 seconds
                ) {
                    handleStopRecording();
                    clearTimer();
                    return prev;
    ...
    const handleAudioPlay = () => {
        if (audioRef.current) {
            // Allow audio to play from its current position
                ...
                `}
                                ...
                                {/* <button>
                                    <audio
                                        ...
                                        onLoadedMetadata={() => {
                                            // doing this to preload the audio file so we can see the length
                                            if (audioRef.current) {
                                                // Load the full length of the audio
