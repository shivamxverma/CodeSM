        ...
        <form onSubmit={submit} className="space-y-4">
          <Field label="Title">
            <input value={f.title} onChange={e => setF({ ...f, title: e.target.value })}
              className="w-full rounded-lg bg-surface border border-border px-3 py-2 text-foreground focus:outline-none focus:ring-2 focus:ring-ring" />
          </Field>
          <Field label="Description">
            <textarea value={f.description} onChange={e => setF({ ...f, description: e.target.value })}
              rows={4} className="w-full rounded-lg bg-surface border border-border px-3 py-2 text-foreground focus:outline-none focus:ring-2 focus:ring-ring" />
          </Field>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            ...
            <Field label="Start Time">
              <input type="datetime-local" value={f.startTime} onChange={e => setF({ ...f, startTime: e.target.value })}
                className="w-full rounded-lg bg-surface border border-border px-3 py-2 text-foreground focus:outline-none" />
            </Field>
            ...
            {f.problems.map((p, i) => (
              <div key={i} className="grid sm:grid-cols-4 gap-2">
                <input placeholder="Problem ID (Mongo _id)" value={p.problemId}
                  ...
                  className="rounded bg-surface border border-border px-3 py-2 text-foreground focus:outline-none" />
