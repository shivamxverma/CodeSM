...
import { getAllProblems } from "@/lib/index";
...
type DificultyType = "All" | "Easy" | "Medium" | "Hard";
type sortType = "A-Z" | "Z-A" | "Easiest" | "Hardest";
...
export default function ProblemPage() {
  ...
  const [difficulty, setDifficulty] = useState<DificultyType>("All");
  ...
  useEffect(() => {
    ...
    async function fetchProblems() {
      try {
        ...
        const problems: Problem[] = res.data.message;
        // console.log(res);
  ...
  const filteredSorted = useMemo(() => {
    ...
    const cmpAZ = (a : Problem, b : Problem) => (a?.title || "").localeCompare(b?.title || "");
    const cmpZA = (a : Problem, b : Problem) => (b?.title || "").localeCompare(a?.title || "");
    const cmpRatingAsc = (a : Problem, b : Problem) => {
    ...
    const cmpRatingDesc = (a : Problem, b : Problem) => {
      const ra = getRating(a), rb = getRating(b);
      if (!Number.isFinite(ra) && !Number.isFinite(rb)) return 0;
      if (!Number.isFinite(ra)) return 1;
      if (!Number.isFinite(rb)) return -1;
  ...
  const diffPill = (_difficulty?: unknown) => {
    const base = "inline-flex items-center gap-1 rounded-full px-2 py-1 text-xs";
    // Provide subtle color variants based on difficulty, callers may pass "Easy" | "Medium" | "Hard"
    if (_difficulty === "Easy") {
      return `${base} bg-emerald-900/20 text-emerald-200 ring-1 ring-emerald-800/20`;
    } else if (_difficulty === "Medium") {
      return `${base} bg-amber-900/20 text-amber-200 ring-1 ring-amber-800/20`;
    } else if (_difficulty === "Hard") {
      return `${base} bg-rose-900/20 text-rose-200 ring-1 ring-rose-800/20`;
    }
        ...
        <div className="mb-6 grid grid-cols-1 gap-3 sm:grid-cols-3">
          ...
          <div className="flex items-center gap-2 rounded-xl border border-white/10 bg-white/5 p-1">
            {["All", "Easy", "Medium", "Hard"].map((d) => (
              <button
                key={d}
                onClick={() => setDifficulty(d as DificultyType)}
