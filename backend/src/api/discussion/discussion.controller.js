...
export const createDiscussion = asyncHandler(async (req, res) => {
    ...
    const query = { title, user: req.user };
    if (problem) {
        query.problem = problem;
    }

    const existingDiscussion = await Discussion.findOne(query);
    ...
    if (problem) {
        discussionData.problem = problem;
    }
...
export const getDiscussions = asyncHandler(async (req, res) => {
    ...
    const cacheKey = problemId ? `discussions_problem_${problemId}` : 'allDiscussions';
    const cachedDiscussions = await redis.get(cacheKey);
    ...
    const cacheExpiry = 60 * 60;
    redis.setex(cacheKey, cacheExpiry, JSON.stringify(discussions));
